...
class Connect
  ...
  def initialize(henv, dsn)
    ...
    temp = @hdbc[0,4]
    henv.add(temp)
    puts "#{temp.unpack('H*')} #{Time.now.to_f} Alloc Connect (#{rc})" if $-W >= 2
  ...
  def self.finalizer_proc(h,henv)
    proc {
      ...
      puts "#{h.unpack('H*')} #{Time.now.to_f} Disconnect (#{rc})"  if $-W >= 2
      rc = RibyCli::SQLFreeHandle(SQL_HANDLE_DBC, h)
      puts "#{h.unpack('H*')} #{Time.now.to_f} Free Connect (#{rc})"  if $-W >= 2
      henv.delete(h)
    }
  end
  def add(handle)
