...
bash-5.1$ cat log/development.log 
   (16.9ms)  SET SCHEMA PROVA2
   (692.5ms)  CREATE TABLE schema_migrations (version VARCHAR(40) NOT NULL PRIMARY KEY)
   (98.9ms)  CREATE TABLE ar_internal_metadata (key VARCHAR(40) NOT NULL PRIMARY KEY, value VARCHAR(40), created_at TIMESTAMP NOT NULL, updated_at TIMESTAMP NOT NULL)
  SQL (110.3ms)  SELECT LOWER(RTRIM(DBILFL)) AS DBILFL, DBIDFT FROM qsys.qadbilfi WHERE dbiLIB = 'PROVA2' AND LOWER(DBILFI) = 'schema_migrations' ORDER BY DBIPOS
...
Migrating to CreateProducts (20220126143513)
   (3.2ms)  SET TRANSACTION ISOLATION LEVEL READ COMMITTED
   (110.2ms)  CREATE OR REPLACE TABLE products (id BIGINT GENERATED BY DEFAULT AS IDENTITY (START WITH 1) PRIMARY KEY, title VARCHAR(40), description VARCHAR(40), price2 smallint, price4 integer, price8 BIGINT, created_at TIMESTAMP NOT NULL, updated_at TIMESTAMP NOT NULL)
   (131.2ms)  COMMENT ON TABLE products IS 'Tabella prodotti'
   (4.8ms)  LABEL   ON TABLE products IS 'Tabella prodotti'
   (8.6ms)  COMMENT ON COLUMN products.title IS 'titolo'
   (5.4ms)  LABEL   ON COLUMN products.title TEXT IS 'titolo'
   (5.4ms)  COMMENT ON COLUMN products.description IS 'descrizione'
   (9.1ms)  LABEL   ON COLUMN products.description TEXT IS 'descrizione'
   (5.3ms)  COMMENT ON COLUMN products.price2 IS 'prezzo piccolo'
   (5.5ms)  LABEL   ON COLUMN products.price2 TEXT IS 'prezzo piccolo'
   (5.2ms)  COMMENT ON COLUMN products.price4 IS 'prezzo medio'
   (5.3ms)  LABEL   ON COLUMN products.price4 TEXT IS 'prezzo medio'
   (5.5ms)  COMMENT ON COLUMN products.price8 IS 'prezzo grande'
   (5.5ms)  LABEL   ON COLUMN products.price8 TEXT IS 'prezzo grande'
  ActiveRecord::SchemaMigration Create (186.4ms)  SELECT version FROM FINAL TABLE (INSERT INTO schema_migrations (version) VALUES (?))
   (83.9ms)  COMMIT
...
Migrating to CreateReviews (20220128134219)
   ...
   (183.5ms)  CREATE TABLE reviews (id BIGINT GENERATED BY DEFAULT AS IDENTITY (START WITH 1) PRIMARY KEY, product_id BIGINT NOT NULL, visits integer, created_at TIMESTAMP NOT NULL, updated_at TIMESTAMP NOT NULL, CONSTRAINT fk_rails_bedd9094d4
FOREIGN KEY (product_id)
  REFERENCES products (id)
)
   (134.0ms)  CREATE INDEX index_reviews_on_product_id ON reviews (product_id)
  ActiveRecord::SchemaMigration Create (39.6ms)  SELECT version FROM FINAL TABLE (INSERT INTO schema_migrations (version) VALUES (?))
   (23.9ms)  COMMIT
   (0.9ms)  SET TRANSACTION ISOLATION LEVEL NO COMMIT
  SQL (50.5ms)  SELECT LOWER(RTRIM(DBILFL)) AS DBILFL, DBIDFT FROM qsys.qadbilfi WHERE dbiLIB = 'PROVA2' AND LOWER(DBILFI) = 'ar_internal_metadata' ORDER BY DBIPOS
  ActiveRecord::InternalMetadata Load (73.5ms)  SELECT ar_internal_metadata.* FROM ar_internal_metadata WHERE ar_internal_metadata.key = ? LIMIT ?
   (0.9ms)  SET TRANSACTION ISOLATION LEVEL READ COMMITTED
  ActiveRecord::InternalMetadata Create (83.0ms)  SELECT key FROM FINAL TABLE (INSERT INTO ar_internal_metadata (key, value, created_at, updated_at) VALUES (?, ?, ?, ?))
   (6.5ms)  COMMIT
   (0.8ms)  SET TRANSACTION ISOLATION LEVEL NO COMMIT
   (3.2ms)  SET SCHEMA PROVA2
  ActiveRecord::SchemaMigration Pluck (23.9ms)  SELECT schema_migrations.version FROM schema_migrations ORDER BY schema_migrations.version ASC
  SQL (41.5ms)  SELECT LOWER(RTRIM(DBILFL)) AS DBILFL, DBIDFT FROM qsys.qadbilfi WHERE dbiLIB = 'PROVA2' AND LOWER(DBILFI) = 'products' ORDER BY DBIPOS
  SQL (33.9ms)  SELECT LOWER(RTRIM(DBILFL)) AS DBILFL, DBIDFT FROM qsys.qadbilfi WHERE dbiLIB = 'PROVA2' AND LOWER(DBILFI) = 'reviews' ORDER BY DBIPOS
```
